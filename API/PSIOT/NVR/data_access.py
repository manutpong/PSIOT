...
def Update_nve(ID_NVR,NVR,NVR_LOC,DESCRIPTION):
    CONSQL = CON_MYSQL()
 
    sql = "update main_device set "
    sql += " ID_NVR = %s ,"
    sql += " NVR_LOC = %s,"
    sql += " DESCRIPTION = %s"
    sql += " where ID = %s"
    recordTuple = (ID_NVR,NVR,NVR_LOC,DESCRIPTION)
    result = CONSQL.Update_query(sql,recordTuple)
    return result
